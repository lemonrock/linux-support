...
pub trait GetUncheckedMut<T> : GetUnchecked<T>
{
	...
	{
		*self.get_unchecked_mut_safe(index) = T
	}
...
impl<'a, T: 'a> GetUncheckedMut<T> for &'a mut [T]
{
	#[inline(always)]
	fn get_unchecked_mut_safe<AUI: AsUsizeIndex>(&mut self, index: AUI) -> &mut T
	{
...
impl<T> GetUncheckedMut<T> for Vec<T>
{
	#[inline(always)]
	fn get_unchecked_mut_safe<AUI: AsUsizeIndex>(&mut self, index: AUI) -> &mut T
	{
...
impl<T> GetUncheckedMut<T> for Box<[T]>
{
	#[inline(always)]
	fn get_unchecked_mut_safe<AUI: AsUsizeIndex>(&mut self, index: AUI) -> &mut T
	{
