	...
	fn current_real_effective_and_saved_set() -> (Self, Self, Self)
	{
		...
		else if likely!(result == -1)
		{
			match SystemCallErrorNumber::from_errno_panic()
			{
				...
				unexpected_error @ _ => unexpected_error!(getresuid, unexpected_error),
			}
		}
		else
		{
			unexpected_result!(getresuid, result)
		}
	}
	...
	fn set_real_effective_and_saved_set(real: Option<Self>, effective: Option<Self>, saved_set: Option<Self>)
	{
		...
		else if likely!(result == -1)
		{
			match SystemCallErrorNumber::from_errno_panic()
			{
				...
				unexpected_error @ _ => unexpected_error!(setresuid, unexpected_error),
			}
		}
		else
		{
			unexpected_result!(setresuid, result)
		}
	}
