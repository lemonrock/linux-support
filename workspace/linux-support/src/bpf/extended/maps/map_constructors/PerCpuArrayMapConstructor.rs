...
pub struct PerCpuArrayMapConstructor<V: Sized>(PhantomData<V>);

impl<V: Sized> MapConstructor for PerCpuArrayMapConstructor<V>
{
	type Map = ArrayMap<V>;
	...
	{
		ArrayMap::new_per_cpu(map_file_descriptors, map_name, parsed_btf_map_data, maximum_entries, access_permissions)
	}
}
