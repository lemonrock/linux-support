...
pub struct WithNumaNodeXdpRedirectSocketArrayMapConstructor;

impl MapConstructor for WithNumaNodeXdpRedirectSocketArrayMapConstructor
{
	type Map = XdpRedirectSocketArrayMap;
	
	type AccessPermissions = XdpAccessPermissions;
	...
	fn construct(map_file_descriptors: &mut FileDescriptorLabelsMap<MapFileDescriptor>, map_name: &MapName, parsed_btf_map_data: Option<&ParsedBtfMapData>, maximum_entries: MaximumEntries, access_permissions: Self::AccessPermissions, _arguments_that_end_up_in_map_flags: Self::InvariantArguments, variable_arguments: Self::VariableArguments) -> Result<Self::Map, MapCreationError>
	{
		...
		XdpRedirectSocketArrayMap::new_xdp_redirect_socket_array(map_file_descriptors, map_name, parsed_btf_map_data, maximum_entries, access_permissions, Some(numa_node))
	}
}
