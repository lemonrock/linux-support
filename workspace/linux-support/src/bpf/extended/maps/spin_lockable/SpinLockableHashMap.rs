...
#[repr(transparent)]
pub struct SpinLockableHashMap<K: Copy, V: 'static + Copy + HasReflectionInformation>(crate::bpf::extended::maps::BpfHashMap<K, SpinLockableValue<V>>);
...
impl<K: Copy, V: 'static + Copy + HasReflectionInformation> SpinLockableHashMap<K, V>
{
	...
	{
		let hash_map = crate::bpf::extended::maps::BpfHashMap::new_system_wide(map_file_descriptors, map_name, Some(parsed_bpf_type_format_map_data), maximum_entries, access_permissions, preallocation, numa_node)?;
		Ok(Self(hash_map))
	}
