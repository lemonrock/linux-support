...
pub struct WithNumaNodeExpressDataPathRedirectSocketArrayMapConstructor;

impl MapConstructor for WithNumaNodeExpressDataPathRedirectSocketArrayMapConstructor
{
	const Singleton: Self = Self;
	
	type Map = ExpressDataPathRedirectSocketArrayMap;
	
	type AccessPermissions = ExpressDataPathAccessPermissions;
	...
	fn construct(map_file_descriptors: &mut FileDescriptorsMap<MapFileDescriptor>, map_name: &MapName, parsed_bpf_type_format_map_data: Option<&ParsedBpfTypeFormatMapData>, maximum_entries: MaximumEntries, access_permissions: Self::AccessPermissions, _arguments_that_end_up_in_map_flags: Self::InvariantArguments, variable_arguments: Self::VariableArguments) -> Result<Self::Map, MapCreationError>
	{
		...
		ExpressDataPathRedirectSocketArrayMap::new_express_data_path_redirect_socket_array(map_file_descriptors, map_name, parsed_bpf_type_format_map_data, maximum_entries, access_permissions, Some(numa_node))
	}
}
