...
pub struct WithNumaNodeStackTraceMapInstructionPointerMapConstructor;

impl MapConstructor for WithNumaNodeStackTraceMapInstructionPointerMapConstructor
{
	const Singleton: Self = Self;
	
	type Map = StackTraceMap<InstructionPointer>;
	
	type AccessPermissions = KernelOnlyAccessPermissions;
	
	type InvariantArguments = ();
	
	type VariableArguments = (StackDepth, NumaNode);
	...
	{
		let (stack_depth, numa_node) = variable_arguments;
		StackTraceMap::new_instruction_pointer(map_file_descriptors, map_name, parsed_bpf_type_format_map_data, maximum_entries, access_permissions, Some(numa_node), stack_depth)
	}
}
