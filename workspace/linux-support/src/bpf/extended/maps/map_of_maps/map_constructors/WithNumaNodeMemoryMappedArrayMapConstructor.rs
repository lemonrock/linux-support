...
impl<V: Copy> MapConstructor for WithNumaNodeMemoryMappedArrayMapConstructor<V>
{
	...
	type VariableArguments = (NumaNode, PageSize);
	...
	{
		let (numa_node, default_page_size) = variable_arguments;
		MemoryMappedArrayMap::new(map_file_descriptors, map_name, parsed_bpf_type_format_map_data, maximum_entries, access_permissions, Some(numa_node), default_page_size)
	}
}
