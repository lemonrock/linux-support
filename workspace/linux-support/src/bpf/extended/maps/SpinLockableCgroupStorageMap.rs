...
impl<V: 'static + Copy + HasReflectionInformation> SpinLockableCgroupStorageMap<V>
{
	/// New system-wide.
	#[inline(always)]
	pub fn new_system_wide(map_file_descriptors: &mut FileDescriptorLabelsMap<MapFileDescriptor>, map_name: &MapName, parsed_btf_map_data: &ParsedBtfMapData, maximum_entries: MaximumEntries, access_permissions: AccessPermissions, numa_node: Option<NumaNode>) -> Result<Self, MapCreationError>
	{
		let cgroup_storage_map = CgroupStorageMap::new_system_wide(map_file_descriptors, map_name, Some(parsed_btf_map_data), maximum_entries, access_permissions, numa_node)?;
