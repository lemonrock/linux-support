...
/// An array map specialized for use with `ReusePortSocket`.
///
/// Values can only be retrieved if `RPSV` is `u64` (weird internal restriction in Linux).
#[derive(Debug, Clone, PartialEq, Eq, PartialOrd, Ord, Hash)]
pub struct ReusePortSocketArrayMap<RPSV: ReusePortSocketValue>
{
	map_file_descriptor: Rc<MapFileDescriptor>,
	maximum_entries: MaximumEntries,
	marker: PhantomData<RPSV>,
}

impl<RPSV: ReusePortSocketValue> CanBeInnerMap for ReusePortSocketArrayMap<RPSV>
{
	#[inline(always)]
...
impl<RPSV: ReusePortSocketValue> ReusePortSocketArrayMap<RPSV>
{
	/// Length.
	#[inline(always)]
	...
	#[inline(always)]
	fn to_value(file_descriptor: &impl ListenerSocketFileDescriptor) -> RPSV
	{
		let raw_fd = file_descriptor.as_raw_fd();
		RPSV::from_raw_fd(raw_fd)
	}
