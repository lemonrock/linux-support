...
pub enum bpf_func_id
{
	...
	/// It can take up to three additional `u64` arguments (in an eBPF helper the total number of arguments is limited to five).
	///
	/// Each time the helper is called, it appends a line to the trace.
	/// Lines are discarded while `/sys/kernel/debug/tracing/trace` is open, use `/sys/kernel/debug/tracing/trace_pipe` to avoid this.
	...
	///
	/// Also, note that`trace_printk()` is slow, and should only be used for debugging purposes.
	...
	///
	/// From a security point of view, this helper uses its own pseudo-random internal state, and cannot be used to infer the seed of other random functions in the kernel.
	...
	/// Note that all programs run with preemption disabled, which means that the SMP processor id is stable during all the execution of the program.
	///
	///
	/// # Returns
	///
	...
	/// `flags` are a combination of `BPF_F_RECOMPUTE_CSUM` (automatically recompute the checksum for the packet after storing the bytes) and
	...
	/// Therefore, at load time, all checks on pointers previously done by the verifier are invalidated and must be/ performed again, if the helper is used in combination with direct packet access.
	///
	///
	/// # Returns
	///
	...
	/// # Description
	///
	/// Recompute the layer 3 (eg IP) checksum for the packet  associated to `skb`.
	/// Computation is incremental, so the helper must know the former value of the header field that was modified (`from`), the new value of this field (`to`), and the number of bytes (2 or 4) for this field, stored in `size`.
	/// Alternatively, it is possible to store the difference between the previous and the new values of the header field in `to`, by setting `from` and `size` to 0.
	/// For both methods, `offset` indicates the location of the IP checksum within the packet.
	///
	...
	/// # Description
	///
	/// Recompute the layer 4 (eg TCP, UDP or ICMP) checksum for the packet associated to `skb`.
	/// Computation is incremental, so the/ helper must know the former value of the header field that was/ modified (`from`), the new value of this field (`to`), and the number of bytes (2 or 4) for this field, stored on the lowest four bits of `flags`.
	/// Alternatively, it is possible to store the difference between the previous and the new values of the header field in `to`, by setting `from` and the four lowest bits of `flags` to 0.
	/// For both methods, `offset` indicates the location of the IP checksum within the packet.
	/// In addition to the size of the field, `flags` can be added (bitwise OR) actual flags.
	...
	/// The same stack frame is used (but values on stack and in registers for the caller are not accessible to the callee).
	...
	/// # Description
	///
	/// Clone and redirect the packet associated to `skb` to another net device of index `ifindex`.
	...
	/// The `comm` attribute contains the name of the executable (excluding the path) for the current task.
	/// The `size_of_buf` must be strictly positive.
	/// On success, the helper makes sure that the `buf` is`NULL`-terminated. On failure, it is filled with zeroes.
	///
	///
	/// # Returns
	///
	...
	/// # Description
	///
	/// Retrieve the `classid` for the current task, ie for the `net_cls` cgroup to which `skb` belongs.
	///
	///
	/// This helper can be used on the traffic control (TC) egress path, but not on ingress.
	///
	/// The `net_cls` cgroup provides an interface to tag network packets based on a user-provided identifier for all traffic coming from the tasks belonging to the related cgroup.
	/// See also the related kernel documentation, available from the Linux sources in file `Documentation/admin-guide/cgroup-v1/net_cls.rst`.
	///
	/// The Linux kernel has two versions for cgroups: there are cgroups v1 and cgroups v2.
	/// Both are available to users, who can use a mixture of them, but note that the `net_cls` cgroup is for cgroup v1 only.
	/// This makes it incompatible with BPF programs run on cgroups, which is a cgroup-v2-only feature (a socket can only hold data for one version of cgroups at a time).
	///
	...
	/// # Description
	///
	/// Push a `vlan_tci`(VLAN tag control information) of protocol `vlan_proto` to the packet associated to `skb`, then update the checksum.
	/// Note that if `vlan_proto` is different from `ETH_P_8021Q` and `ETH_P_8021AD`, it is considered to be `ETH_P_8021Q`.
	///
	/// A call to this helper is susceptible to change the underlying packet buffer.
	/// Therefore, at load time, all checks on pointers previously done by the verifier are invalidated and must be/ performed again, if the helper is used in combination with direct packet access.
	///
	...
	///
	/// Get tunnel metadata.
	/// This helper takes a pointer `key` to an empty `struct bpf_tunnel_key` of `size`, that will be filled with tunnel metadata for the packet associated to `skb`.
	/// The `flags` can be set to `BPF_F_TUNINFO_IPV6`, which indicates that the tunnel is based on IPv6 protocol instead of IPv4.
	///
	/// The `struct bpf_tunnel_key` is an object that generalizes the principal parameters used by various tunneling protocols into a single struct.
	/// This way, it can be used to easily make a decision based on the contents of the encapsulation header, "summarized" in this struct.
	/// In particular, it holds the IP address of the remote end (IPv4 or IPv6, depending on the case)` in `key->remote_ipv4` or `key->remote_ipv6`.
	/// Also, this struct exposes the `key->tunnel_id`, which is generally mapped to a VNI (Virtual Network Identifier), making/ it programmable together with the `bpf_skb_set_tunnel_key()` helper.
	///
	/// Let's imagine that the following code is part of a program attached to the traffic control (TC) ingress interface, on one end of a GRE tunnel, and is supposed to filter out all messages coming from remote ends with IPv4 address other than `10.0.0.1`:-
	///
	/// ```bash
	/// int ret;
	...
	/// # Description
	///
	/// Populate tunnel metadata for packet associated to `skb` tunnel metadata is set to the contents of `key`, of `size`.
	/// The `flags` can be set to a combination of the following values:-
	///
	...
	/// This helper relies on a `map` of type `BPF_MAP_TYPE_PERF_EVENT_ARRAY`.
	/// The nature of the perf event counter is selected when `map` is updated with perf event file descriptors.
	/// The `map` is an array whose size is the number of available CPUs, and each cell contains a value relative to one CPU.
	/// The value to retrieve is indicated by `flags`, that contains the index of the CPU to look up, masked with `BPF_F_INDEX_MASK`.
	/// Alternatively, `flags` can be set to `BPF_F_CURRENT_CPU` to indicate that the value for the current CPU should be retrieved.
	///
	/// Note that before Linux 4.13, only hardware perf event can be retrieved.
	///
	/// Also, be aware that the newer helper `bpf_perf_event_read_value()` is recommended over `bpf_perf_event_read()` in general.
	/// The latter has some ABI quirks where error and counter value are used as a return code (which is wrong to do since ranges may overlap).
	/// This issue is fixed with `bpf_perf_event_read_value()`, which at the same time provides more features over the `bpf_perf_event_read()` interface.
	...
	/// This helper is somewhat similar to `bpf_clone_redirect()`, except that the packet is not cloned, which provides increased performance.
	///
	/// Except for XDP, both ingress and egress interfaces can be used for redirection.
	/// The `BPF_F_INGRESS` value in `flags` is used to make the distinction (ingress path is selected if the flag is present, egress path otherwise).
	/// Currently, XDP only supports redirection to the egress interface, and accepts no flag at all.
	///
	...
	///
	/// This helper is available only if the kernel was compiled with the `CONFIG_IP_ROUTE_CLASSID` configuration option.
	///
	///
	/// # Returns
	///
	...
	/// # Description
	///
	/// Write raw `data` blob into a special BPF perf event held by `map` of type `BPF_MAP_TYPE_PERF_EVENT_ARRAY`.
	/// This perf event must have the following attributes: `PERF_SAMPLE_RAW` as `sample_type`, `PERF_TYPE_SOFTWARE` as `type`, and `PERF_COUNT_SW_BPF_OUTPUT` as `config`.
	///
	...
	///
	/// On user space, a program willing to read the values needs to call `perf_event_open()` on the perf event (either for one or for all CPUs) and to store the file descriptor into the `map`.
	/// This must be done before the eBPF program can send data into it.
	/// An example is available in file `samples/bpf/trace_output_user.c` in the Linux kernel source tree (the eBPF program counterpart is in `samples/bpf/trace_output_kern.c`).
	///
	...
	///
	/// Note that this helper is not restricted to tracing use cases and can be used with programs attached to TC or XDP as well, where it allows for passing data to user space listeners.
	/// Data can be:-
	///
	/// * Only custom structs,
	...
	/// It can be used to load `len` bytes from `offset` from the packet associated to `skb`, into the buffer pointed by `to`.
	///
	/// Since Linux 4.7, usage of this helper has mostly been replaced by "direct packet access", enabling packet data to be manipulated with `skb->data` and `skb->data_end` pointing respectively to the first byte of packet data and to the byte after the last byte of packet data.
	...
	/// To achieve this, the helper needs `ctx`, which is a pointer to the context on which the tracing program is executed, and a pointer to a `map` of type `BPF_MAP_TYPE_STACK_TRACE`.
	///
	...
	///
	/// The stack id retrieved is a 32 bit long integer handle which can be further combined with other data (including other stack ids) and used as a key into maps.
	/// This can be useful for generating a variety of graphs (such as flame graphs or off-cpu graphs).
	///
	/// For walking a stack, this helper is an improvement over `bpf_probe_read()`, which can be used with unrolled loops but is not efficient and consumes a lot of eBPF instructions.
	/// Instead, `bpf_get_stackid()` can collect up to `PERF_MAX_STACK_DEPTH` both kernel and user frames.
	...
	/// # Description
	///
	/// Compute a checksum difference, from the raw buffer pointed by  `from`, of length `from_size` (that must be a multiple of 4), towards the raw buffer pointed by `to`, of size `to_size` (same remark).
	/// An optional `seed` can be added to the value (this can be cascaded, the `seed` may come from a previous call to the helper).
	///
	...
	///
	/// This helper can be used with encapsulation devices that can operate in "collect metadata" mode (please refer to the related note in the description of `bpf_skb_get_tunnel_key()` for more details).
	/// A particular example where this can be used is in combination with the Geneve encapsulation protocol, where it allows for pushing (with the `bpf_skb_get_tunnel_opt()` helper)` and retrieving arbitrary TLVs (Type-Length-Value headers) from the eBPF program.
	...
	/// Currently supported are transition from IPv4 to IPv6, and from IPv6 to IPv4.
	/// The helper takes care of the groundwork for the transition, including resizing the socket buffer.
	/// The eBPF program is expected to fill the new headers, if any, via `skb_store_bytes()` and to recompute the checksums with `bpf_l3_csum_replace()` and `bpf_l4_csum_replace()`.
	/// The main case for this helper is to perform NAT64 operations out of an eBPF program.
	///
	/// Internally, the GSO type is marked as dodgy so that headers are checked and segments are recalculated by the GSO/GRO engine.
	...
	/// This comes down to setting `skb->pkt_type` to `type`, except the eBPF program does not have a write access to `skb->pkt_type` beside this helper.
	...
	///
	/// Note that `type` only allows certain values.
	...
	/// If it is not set, in particular if the hash was cleared due to mangling, recompute this hash.
	...
	/// It only works for threads that are in user context, and `dst` must be a valid user space address.
	///
	...
	///
	/// Keep in mind that this feature is meant for experiments, and it has a risk of crashing the system and running programs.
	/// Therefore, when an eBPF program using this helper is attached, a warning including PID and process name is printed to kernel logs.
	///
	///
	/// # Returns
	///
	...
	/// # Description
	///
	/// Check whether the probe is being run is the context of a given subset of the cgroup2 hierarchy.
	/// The cgroup2 to test is held by `map` of type `BPF_MAP_TYPE_CGROUP_ARRAY`, at `index`.
	///
	///
	/// # Returns
	///
	...
	/// # Description
	///
	/// Resize (trim or grow) the packet associated to `skb` to the new `len`.
	/// The `flags` are reserved for future usage, and must be left at zero.
	///
	/// The basic idea is that the helper performs the needed work to change the size of the packet, then the eBPF program rewrites the rest via helpers like `bpf_skb_store_bytes()`, `bpf_l3_csum_replace()`, `bpf_l3_csum_replace()` and others.
	/// This helper is a slow path utility intended for replies with control messages.
	/// And, because it is targeted for slow path, the helper itself can afford to be slow: it implicitly linearizes, unclones and drops offloads from the `skb`.
	///
	/// A call to this helper is susceptible to change the underlying packet buffer.
	/// Therefore, at load time, all checks on pointers previously done by the verifier are invalidated and must be/ performed again, if the helper is used in combination with direct packet access.
	///
	...
	/// # Description
	///
	/// Pull in non-linear data in case the `skb` is non-linear and not all of `len` are part of the linear section.
	/// Make `len` bytes from `skb` readable and writable.
	/// If a zero value is passed for `len`, then the whole length of the `skb` is pulled.
	///
	/// This helper is only needed for reading and writing with direct packet access.
	///
	/// For direct packet access, testing that offsets to access are within packet boundaries (test on `skb->data_end`) is susceptible to fail if offsets are invalid, or if the requested data is in non-linear parts of the `skb`.
	/// On failure the program can just bail out, or in the case of a non-linear buffer, use a helper to make the data available.
	/// The `bpf_skb_load_bytes()` helper is a first solution to access the data.
	...
	/// As this needs to be an invariant for the write part only, the verifier detects writes and adds a prologue that is calling `bpf_skb_pull_data()` to effectively unclone the `skb` from the very beginning in case it is indeed cloned.
	///
	/// A call to this helper is susceptible to change the underlying packet buffer.
	/// Therefore, at load time, all checks on pointers previously done by the verifier are invalidated and must be/ performed again, if the helper is used in combination with direct packet access.
	///
	...
	/// # Description
	///
	/// Add the checksum `csum` into `skb->csum` in case the driver has supplied a checksum for the entire packet into that field.
	/// Return an error otherwise.
	/// This helper is intended to be used in combination with `bpf_csum_diff()`, in particular when the checksum needs to be updated after data has been written into the packet through direct packet access.
	///
	///
	/// # Returns
	///
	...
	/// The primary use case for this helper is the selection of sockets for the local NUMA node, when the program is attached to sockets using the `SO_ATTACH_REUSEPORT_EBPF` option (see also `man 7 socket`), but the helper is also available to other eBPF program types, similarly to `bpf_get_smp_processor_id()`.
	///
	///
	/// # Returns
	///
	...
	/// # Description
	///
	/// Grows headroom of packet associated to `skb` and adjusts the offset of the MAC header accordingly, adding `len` bytes of space.
	...
	/// # Description
	///
	/// Copy a`NULL` terminated string from an unsafe kernel address `unsafe_ptr` to `dst`.
	...
	/// # Description
	///
	/// If the `struct sk_buff` pointed by `skb`, or the `struct bpf_sock_addr` pointed to `ctx`, or the `struct bpf_sock_ops` pointed to by `ctx` has a known socket, retrieve the cookie (generated by the kernel) of this socket.
	/// If no cookie has been set yet, generate a new cookie.
	/// Once generated, the socket cookie remains stable for the life of the socket.
	/// This helper can be useful for monitoring per socket networking traffic statistics as it provides a global socket identifier that can be assumed unique.
	///
	///
	/// ## Returns
	///
	...
	/// # Description
	///
	/// Emulate a call to `setsockopt()` on the socket associated to `bpf_socket`, which must be a full socket
	/// The `level` at which the option resides and the name `optname` of the option must be specified, see `man 2 setsockopt` for more information.
	/// The option value of length `optlen` is pointed by `optval`.
	///
	/// This helper actually implements a subset of `setsockopt()`.
	/// It supports the following `level`s:-
	///
	/// * `SOL_SOCKET`, which supports the `optname`s:-
	///   * `SO_RCVBUF`.
	///   * `SO_SNDBUF`.
	///   * `SO_MAX_PACING_RATE`.
	///   * `SO_PRIORITY`.
	///   * `SO_RCVLOWAT`.
	///   * `SO_MARK`.
	/// * `IPPROTO_TCP`, which supports the `optname`s:-
	///   * `TCP_CONGESTION`.
	///   * `TCP_BPF_IW`.
	///   * `TCP_BPF_SNDCWND_CLAMP`.
	/// * `IPPROTO_IP`, which supports the `optname`s:-
	///   * `IP_TOS`.
	/// * `IPPROTO_IPV6`, which supports the `optname`s:-
	///   * `IPV6_TCLASS`.
	///
	///
	/// # Returns
	...
	/// * `BPF_F_ADJ_ROOM_ENCAP_L3_IPV4`: Any new space is reserved to hold a tunnel header. Configure skb offsets and other fields accordingly.
	...
	/// * `BPF_F_ADJ_ROOM_ENCAP_L4_GRE`: Use with `ENCAP_L3` flags to further specify the tunnel type.
	...
	/// Depending on its type, this `map` can contain references to net devices (for forwarding packets through other ports), or to CPUs (for redirecting XDP frames to another CPU; but this is only implemented for native XDP (with driver
	/// support) as of this writing).
	///
	/// The lower two bits of `flags` are used as the return code if the map lookup fails.
	/// This is so that the return value can be one of the XDP program return codes up to `XDP_TX`, as chosen by the caller.
	/// Any higher bits in the `flags` argument must be unset.
	///
	...
	/// # Description
	///
	/// Redirect the packet to the socket referenced by `map` (of type `BPF_MAP_TYPE_SOCKMAP`) at index `key`.
	/// Both ingress and egress interfaces can be used for redirection.
	/// The `BPF_F_INGRESS` value in `flags` is used to make the distinction (ingress path is selected if the flag is present, egress path otherwise).
	/// This is the only flag supported for now.
	///
	...
	/// The `skops` is used as a new value for the entry associated to `key`.
	/// `flags` is one of:-
	///
	/// * `BPF_NOEXIST`: The entry for `key` must not exist in the map.
	/// * `BPF_EXIST`: The entry for `key` must already exist in the map.
	/// * `BPF_ANY`: No condition on the existence of the entry for `key`.
	///
	/// If the `map` has eBPF programs (parser and verdict), those will be inherited by the socket being added.
	/// If the socket is already attached to eBPF programs, this results in an error.
	///
	///
	/// # Returns
	///
	...
	/// # Description
	///
	/// Adjust the address pointed by `xdp_md->data_meta` by `delta` (which can be positive or negative).
	...
	///
	/// The use of `xdp_md->data_meta` is optional and programs are not required to use it.
	/// The rationale is that when the packet is processed with XDP (eg as DoS filter), it is possible to push further meta data along with it before passing to the stack, and to give the guarantee that an ingress eBPF program attached as a TC classifier on the same device can pick this up for further post-processing.
	/// Since TC works with socket buffers, it remains possible to set from XDP the `mark` or `priority` pointers, or other pointers for the socket buffer.
	/// Having this scratch space generic and programmable allows for more flexibility as the user is free to store whatever meta data they need.
	///
	/// A call to this helper is susceptible to change the underlying packet buffer.
	/// Therefore, at load time, all checks on pointers previously done by the verifier are invalidated and must be performed again, if the helper is used in combination with direct packet access.
	///
	...
	/// Alternatively, `flags` can be set to `BPF_F_CURRENT_CPU` to indicate that the value for the current CPU should be retrieved.
	///
	/// This helper behaves in a way close to `bpf_perf_event_read()` helper, save that instead of just returning the value observed, it fills the `buf` structure.
	/// This allows for additional data to be retrieved: in particular, the enabled and running times (in `buf->enabled` and `buf->running`, respectively) are copied.
	...
	///
	/// These values are interesting, because hardware PMU (Performance Monitoring Unit) counters are limited resources.
	/// When there are more PMU based perf events opened than available counters, kernel will multiplex these events so each event gets certain percentage (but not all) of the PMU time.
	/// In case that multiplexing happens, the number of samples or counter value will not reflect the case compared to when no multiplexing occurs.
	/// This makes comparison between different runs difficult
	/// Typically, the counter value should be normalized before comparing to other experiments.
	/// The usual normalization is done as follows:-
	///
	/// ```bash
	/// normalized_counter = counter * t_enabled / t_running
	/// ```
	///
	/// Where `t_enabled` is the time enabled for event and `t_running` is the time running for event since last normalization.
	/// The enabled and running times are accumulated since the perf event open.
	/// To achieve scaling factor between two invocations of an/ eBPF program, users can can use CPU id as the key (which is typical for perf array usage model) to remember the previous value and do the calculation inside the eBPF program.
	///
	///
	/// # Returns
	///
	...
	/// # Description
	///
	/// For en eBPF program attached to a perf event, retrieve the value of the event counter associated to `ctx` and store it in the structure pointed by `buf` and of size `buf_size`.
	/// Enabled and running times are also stored in the structure (see description of helper `bpf_perf_event_read_value()` for more details).
	///
	///
	/// # Returns
	///
	...
	/// # Description
	///
	/// Emulate a call to `getsockopt()` on the socket associated to `bpf_socket`, which must be a full socket.
	/// The `level` at which the option resides and the name `optname` of the option must be specified, see `man 2 getsockopt` for more information.
	/// The retrieved value is stored in the structure pointed by `optval` and of length `optlen`.
	///
	...
	/// # Description
	///
	/// Used for error injection, this helper uses kprobes to override the return value of the probed function, and to set it to `rc`.
	/// The first argument is the context `regs` on which the kprobe works.
	///
	/// This helper works by setting setting the PC (program counter) to an override function which is run in place of the original probed function.
	/// This means the probed function is not run at all.
	/// The replacement function just returns with the required value.
	///
	/// This helper has security implications, and thus is subject to/ restrictions.
	/// It is only available if the kernel was compiled with the `CONFIG_BPF_KPROBE_OVERRIDE` configuration option, and in this case it only works on functions tagged with `ALLOW_ERROR_INJECTION` in the kernel code.
	///
	/// Also, the helper is only available for the architectures having the `CONFIG_FUNCTION_ERROR_INJECTION` option.
	/// As of this writing, x86 and x86_64 architectures are the only ones to support this feature.
	///
	///
	/// # Returns
	///
	...
	///
	/// The primary use of this field is to determine if there should be calls to eBPF programs of type `BPF_PROG_TYPE_SOCK_OPS` at various points in the TCP code.
	/// A program of the same type can change its value, per connection and as necessary, when the connection is established.
	/// This field is directly accessible for reading, but this helper must be used for updates in order to return an error if an eBPF program tries to set a callback that is not supported in the current kernel.
	///
	...
	///
	/// Therefore, this function can be used to clear a callback flag by setting the appropriate bit to zero, eg to disable the RTO callback:-
	///
	/// ```bash
	/// `bpf_sock_ops_cb_flags_set(bpf_sock, bpf_sock->bpf_sock_ops_cb_flags & ~BPF_SOCK_OPS_RTO_CB_FLAG)`
	/// ```
	///
	/// Here are some examples of where one could call such an eBPF program:-
	///
	/// * When RTO fires.
	...
	/// If the message `msg` is allowed to pass (ie if the verdict eBPF program returns `SK_PASS`), redirect it to the socket referenced by `map` (of type/ `BPF_MAP_TYPE_SOCKMAP`) at index `key`.
	/// Both ingress and egress interfaces can be used for redirection.
	/// The `BPF_F_INGRESS` value in `flags` is used to make the distinction (ingress path is selected if the flag is present, egress path otherwise).
	/// This is the only flag supported for now.
	///
	...
	/// If `bytes` is smaller than the current data being processed from a `sendmsg()` or `sendfile()` system call, the first `bytes` will be sent and the eBPF program will be re-run with the pointer for start of data pointing to byte number `bytes + 1`.
	/// If `bytes` is larger than the current data being processed, then the eBPF verdict will be applied to multiple `sendmsg()` or `sendfile()` calls until `bytes` are consumed.
	///
	...
	/// The extreme case would be a user calling `sendmsg()` repeatedly with 1-byte long message segments.
	/// Obviously, this is bad for performance, but it is still valid.
	/// If the eBPF program needs `bytes` bytes to validate a header, this helper can be used to prevent the eBPF program to be called again until `bytes` have been accumulated.
	///
	///
	/// # Returns
	///
	...
	/// # Description
	///
	/// Bind the socket associated to `ctx` to the address pointed by `addr`, of length `addr_len`.
	/// This allows for making outgoing connection from the desired IP address, which can be useful for example when all processes inside a cgroup should use one single IP address on a host that has multiple IP configured.
	///
	/// This helper works for IPv4 and IPv6, TCP and UDP sockets.
	/// The domain (`addr->sa_family`) must be `AF_INET` or `AF_INET6`.
	/// Looking for a free port to bind to can be expensive, therefore binding to port is not permitted by the helper: `addr->sin_port` (for IPv4) or `addr->sin6_port` (for IPv6) must be set to zero.
	///
	///
	/// # Returns
	///
	...
	/// It is only possible to shrink the packet as of this writing, therefore `delta` must be a negative integer.
	///
	/// A call to this helper is susceptible to change the underlying packet buffer.
	/// Therefore, at load time, all checks on pointers previously done by the verifier are invalidated and must be performed again, if the helper is used in combination with direct packet access.
	///
	...
	/// # Description
	///
	/// Retrieve the XFRM state (IP transform framework, see also `man 8 ip-xfrm`) at `index` in XFRM "security path" for `skb`.
	///
	...
	/// # Description
	///
	/// This helper is similar to `bpf_skb_load_bytes()` in that it provides an easy way to load `len` bytes from `offset` from the packet associated to `skb`, into the buffer pointed by `to`.
	/// The difference to `bpf_skb_load_bytes()` is that a fifth argument `start_header` exists in order to select a base offset to start from.
	/// `start_header` can be one of:-
	///
	/// * `BPF_HDR_START_MAC`: Base offset to load data from is `skb`'s mac header.
	/// * `BPF_HDR_START_NET`: Base offset to load data from is `skb`'s network header.
	///
	...
	///
	/// This helper is used in programs implementing policies at the socket level.
	/// If the message `msg` is allowed to pass (ie if the verdict eBPF program returns `SK_PASS`), redirect it to the socket referenced by `map` (of type `BPF_MAP_TYPE_SOCKHASH`) using hash `key`.
	/// Both ingress and egress interfaces can be used for redirection.
	/// The `BPF_F_INGRESS` value in `flags` is used to make the distinction (ingress path is selected if the flag is present, egress path otherwise).
	/// This is the only flag supported for now.
	///
	...
	/// This header is provided in the buffer at address `hdr`, with `len` its size in bytes.
	...
	///
	/// Store `len` bytes from address `from` into the packet associated to `skb`, at `offset`.
	/// Only the flags, tag and TLVs inside the outermost IPv6 Segment Routing Header can be modified through this helper.
	///
	/// A call to this helper is susceptible to change the underlying packet buffer.
	/// Therefore, at load time, all checks on pointers previously done by the verifier are invalidated and must be performed again, if the helper is used in combination with direct packet access.
	///
	...
	/// # Description
	///
	/// Adjust the size allocated to TLVs in the outermost IPv6 Segment Routing Header contained in the packet associated to `skb`, at position `offset` by `delta` bytes.
	/// Only offsets after the segments are accepted.
	/// `delta` can be positive (growing) as well as negative (shrinking).
	///
	/// A call to this helper is susceptible to change the underlying packet buffer.
	/// Therefore, at load time, all checks on pointers previously done by the verifier are invalidated and must be performed again, if the helper is used in combination with direct packet access.
	///
	...
	/// # Description
	///
	/// Apply an IPv6 Segment Routing action of type `action` to the packet associated to `skb`.
	/// Each action takes a parameter contained at address `param`, and of length `param_len` bytes.
	...
	/// # Description
	///
	/// This helper is used in programs implementing IR decoding, to report a successfully decoded key press with `scancode`, `toggle` value in the given `protocol`.
	/// The `scancode` will be translated to a keycode using the rc keymap, and reported as an input key down event.
	/// After a period a key up event is generated.
	/// This period can be extended by calling either `bpf_rc_keydown()` again with the same values, or calling `bpf_rc_repeat()`.
	///
	...
	/// This is roughly similar to the `bpf_get_cgroup_classid()` helper for cgroup v1 by providing a tag resp. identifier that can be matched on or used for map lookups eg to implement policy.
	/// The cgroup v2 id of a given path in the hierarchy is exposed in user space through the f_handle API in order to get to the same 64-bit id.
	///
	...
	/// The type and the size of the local storage is defined by the `map` argument.
	/// The `flags` meaning is specific for each map type, and has to be `0` for cgroup local storage.
	///
	...
	///
	/// A running program should take care to ensure synchronization is used, eg, by using the  `BPF_STX_XADD` instruction to alter the shared data.
	///
	///
	/// # Returns
	///
	...
	/// # Description
	///
	/// Return id of cgroup v2 that is ancestor of cgroup associated with the `skb` at the `ancestor_level`.
	/// The root cgroup is at `ancestor_level` zero and each step down the hierarchy increments the level.
	/// If `ancestor_level` == level of cgroup associated with `skb`, then return value will be same as that of `bpf_skb_cgroup_id()`.
	///
	...
	/// # Description
	///
	/// Look for TCP socket matching `tuple`, optionally in a child or network namespace `netns`.
	/// The return value must be checked, and if non-`NULL`, released via `bpf_sk_release()`.
	///
	...
	///
	/// * `size_of_val(tuple->ipv4)`: Look for an IPv4 socket.
	/// * `size_of_val(tuple->ipv6)`: Look for an IPv6 socket.
	///
	/// If the `netns` is a negative signed 32-bit integer, then the socket lookup table in the netns associated with the `ctx` will will be used.
	/// For the TC hooks, this is the `netns` of the device in the skb.
	/// For socket hooks, this is the `netns` of the socket.
	/// If `netns` is any other signed 32-bit value greater than or/ equal to zero then it specifies the ID of the `netns` relative to the netns associated with the `ctx`.
	/// `netns` values beyond the range of 32-bit integers are reserved for future use.
	///
	/// All values for `flags` are reserved for future usage, and must be left at zero.
	///
	...
	/// # Description
	///
	/// Look for UDP socket matching `tuple`, optionally in a child network namespace `netns`.
	/// The return value must be checked, and if non-`NULL`, released via `bpf_sk_release()`.
	///
	/// The `ctx` should point to the context of the program, such as the skb or socket (depending on the hook in use).
	/// This is used to determine the base network namespace for the lookup.
	...
	/// `sock` must be a non-`NULL` pointer that was returned from either `bpf_sk_lookup_tcp()` or `bpf_sk_lookup_udp()`.
	///
	///
	/// # Returns
	///
	...
	///
	/// If a program of type `BPF_PROG_TYPE_SK_MSG` is run on a `msg` it may want to insert metadata or options into the `msg`.
	/// This can later be read and used by any of the lower layer BPF hooks.
	///
	...
	/// This may result in `ENOMEM` errors under certain situations if an allocation and copy are required due to a full ring buffer.
	/// However, the helper will try to avoid doing the allocation if possible.
	...
	/// # Description
	///
	/// Acquire a spinlock represented by the pointer `lock`, which is stored as part of a value of a map.
	/// Taking the lock allows to safely update the rest of the fields in that value.
	/// The spinlock can (and must) later be released with a call to `bpf_spin_unlock(lock)`.
	///
	...
	/// * The BPF program can access `struct bpf_spin_lock` only via the `bpf_spin_lock()` and `bpf_spin_unlock()`.
	///   helpers. Loading or storing data into the `struct bpf_spin_lock *lock;` field of a map is not allowed.
	/// * To use the `bpf_spin_lock()` helper, the BTF description of the map value must be a struct and have `struct bpf_spin_lock *anyname;` field at the top level.
	...
	/// # Description
	///
	/// Set the ECN (Explicit Congestion Notification) field of IP header to `CE` (Congestion Encountered) if current value is `ECT` (ECN Capable Transport).
	...
	/// # Description
	///
	/// Look for TCP socket matching `tuple`, optionally in a child network namespace `netns`.
	/// The return value must be checked, and if non-`NULL`, released via `bpf_sk_release()`.
	///
	...
	///
	/// `iph` points to the start of the IPv4 or IPv6 header, while `iph_len` contains `size_of::<struct iphdr>()` or `size_of::<struct ip6hdr>()`.
	///
	/// `th` points to the start of the TCP header, while `th_len` contains `size_of::<struct tcphdr>()`).
	///
	///
	/// # Returns
	///
	...
	///
	/// The buffer is always `NULL` terminated, unless it's zero-sized.
	///
	/// If `flags` is zero, full name (eg `net/ipv4/tcp_mem`) is copied.
	/// Use `BPF_F_SYSCTL_BASE_NAME` flag to copy base name only (eg `tcp_mem`).
	///
	///
	/// # Returns
	///
	...
	/// # Description
	///
	/// Get current value of sysctl as it is presented in `/proc/sys` (including newline, etc), and copy it as a string into provided by program buffer `buf` of size `buf_len`.
	///
	/// The whole value is copied, no matter what file position user space issued eg sys_read at.
	///
	/// The buffer is always`NULL` terminated, unless it's zero-sized.
	///
	///
	...
	///
	/// User space may write new value at `file position > 0`.
	/// To override the whole sysctl value file position should be set to zero.
	///
	///
	/// # Returns
	///
	...
	///
	/// The string may begin with an arbitrary amount of white space (as determined by `man 3 isspace()`) followed by a single optional `-` sign.
	///
	/// Five least significant bits of `flags` encode base, other bits are currently unused.
	///
	/// Base must be either 8, 10, 16 or 0 to detect it automatically similar to user space `man 3 strtol()`.
	///
	///
	/// # Returns
	///
	...
	///
	/// The string may begin with an arbitrary amount of white space (as determined by `man 3 isspace()`).
	///
	/// Five least significant bits of `flags` encode base, other bits are currently unused.
	///
	/// Base must be either 8, 10, 16 or 0 to detect it automatically similar to user space `man 3 strtol()`.
	...
	///
	/// Logically, it could be thought of getting the value from a `map` with `sk` as the `key`.
	/// From this perspective, the usage is not much different from `bpf_map_lookup_elem(map, &sk)` except this helper enforces the key must be a full socket and the map must be a `BPF_MAP_TYPE_SK_STORAGE` also.
	///
	/// Underneath, the value is stored locally at `sk` instead of the `map`.
	/// The `map` is used as the bpf-local-storage "type".
	/// The bpf-local-storage "type" (ie the `map`) is searched against all bpf-local-storages residing at `sk`.
	///
	...
	/// `value` can be used together with `BPF_SK_STORAGE_GET_F_CREATE` to specify the initial value of a bpf-local-storage.
	/// If `value` is `NULL`, the new bpf-local-storage will be zero initialized.
	///
	///
	/// # Returns
	///
	...
	/// The `size` should include the terminating`NULL` byte.
	/// In case the string length is smaller than `size`, the target is not padded with further `NULL` bytes.
	/// If the string length is larger than `size`, just `size - 1` bytes are copied and the last byte is set to `NULL`.
	///
	/// On success, the length of the copied string is returned.
	/// This makes this helper useful in tracing programs for reading strings, and more importantly to get its length at runtime.
	...
	/// {
	///     // PATHLEN is defined to 256.
	///     char buf[PATHLEN];
	///
	///     int res = bpf_probe_read_user_str(buf, sizeof(buf), ctx->di);
	///
	///     // Consume buf, for example push it to userspace via bpf_perf_event_output();
	///     // we can use res (the string length) as event size, after checking its boundaries.
	/// }
	/// ```
	///
	...
	/// On error, a negative value.
	///
	/// The `flags` can be set to `BPF_F_GET_BRANCH_RECORDS_SIZE` to instead return the number of bytes required to store all the branch entries.
	/// If this flag is set, `buf` may be `NULL`.
	///
	...
	///
	/// This helper is similar to `bpf_perf_eventoutput()` but is restricted to raw tracepoint BPF programs.
	///
	///
	/// # Returns
	///
	...
	/// # Description
	///
	/// Retrieve the cookie (generated by the kernel) of the network namespace the input `ctx` is associated with.
	/// The network namespace cookie remains stable for its lifetime and provides a global identifier that can be assumed unique.
	/// If `ctx` is `NULL`, then the helper returns the cookie for the initial network namespace.
	/// The cookie itself is very similar to that of the `bpf_get_socket_cookie()` helper, but for network namespaces instead of sockets.
	///
	///
	/// # Returns
	///
	...
	/// The root cgroup is at `ancestor_level` zero and each step down the hierarchy increments the level.
	/// If `ancestor_level` == level of cgroup associated with the current task, then return value will be the same as that of `bpf_get_current_cgroup_id()`.
	///
