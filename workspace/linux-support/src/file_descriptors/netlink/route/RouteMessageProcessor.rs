...
pub trait RouteMessageProcessor
{
	...
	{
		debug_assert_eq!(message_type, Self::ExpectedMessageType);
		...
		let mut potential_message_attribute_pointer = unsafe { data.as_ptr().add(NETLINK_ALIGN::<Self::Header>()) as *const rtattr };
