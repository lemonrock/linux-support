...
pub(crate) struct GetInternetProtocolVersion6MulticastAddressMessageProcessor;

impl MessageProcessor for GetInternetProtocolVersion6MulticastAddressMessageProcessor
{
	...
	fn process_message_attribute(&self, message_attribute: &rtattr<Self::NAT>, processing_message_state: &mut Self::ProcessingMessageState) -> Result<(), String>
	{
		...
		match message_attribute.type_()
		{
			...
			// Optional.
			// Other.
			(false, false, IFA_MULTICAST) => set_address_field(&mut processing_message_state.other_cast_address, message_attribute, rtattr::get_attribute_value_raw_protocol_address)?,
