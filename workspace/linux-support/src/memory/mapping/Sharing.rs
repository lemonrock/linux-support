...
pub enum Sharing
{
	...
	///
	/// Changes to memory backed by files are only guaranteed to become visible once `msync()` has been called.
	///
	...
	///
	/// Shared file mappings with this flag provide the guarantee that while some memory is mapped as writable in the address space of the process, it will be visible in the same file at the same offset even after the system crashes or is rebooted.
