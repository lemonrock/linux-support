...
impl MemoryPolicy
{
	...
	pub fn get_current_thread_memory_policy() -> (Self, MemoryPolicyDynamism)
	{
		...
		Self::guard_result(system_call_get_mempolicy(&mut mode, null_mut(), 0, null(), GetMemoryPolicyFlags::empty()));
	...
	pub fn get_current_thread_valid_numa_nodes_for_set_memory_policy_and_mbind() -> NumaNodes
	{
		...
		Self::guard_result(system_call_get_mempolicy(null_mut(), pointer, length, null(), GetMemoryPolicyFlags::MPOL_F_MEMS_ALLOWED));
	...
	pub fn get_memory_policy_for_memory(address: NonNull<u8>) -> (Self, MemoryPolicyDynamism, BitSet<NumaNode>)
	{
		...
		Self::guard_result(system_call_get_mempolicy(&mut mode, pointer, length, address.as_ptr() as *mut c_void, GetMemoryPolicyFlags::MPOL_F_ADDR));
	...
	pub fn get_numa_node_for_memory(address: NonNull<u8>) -> NumaNode
	{
		...
		Self::guard_result(system_call_get_mempolicy(&mut mode, null_mut(), 0, address.as_ptr() as *mut c_void, GetMemoryPolicyFlags::MPOL_F_NODE | GetMemoryPolicyFlags::MPOL_F_ADDR));
	...
	pub fn get_current_thread_numa_node_for_next_interleaved_internal_kernel_page() -> Option<NumaNode>
	{
		...
		let mut mode = unsafe_uninitialized();
		Self::guard_result(system_call_get_mempolicy(&mut mode, null_mut(), 0, null(), GetMemoryPolicyFlags::MPOL_F_NODE));
	...
	fn guard_result(result: isize)
	{
		...
		else if likely!(result == -1)
		{
			match SystemCallErrorNumber::from_errno_panic()
			{
				...
				unexpected_error @ _ => unexpected_error!(get_mempolicy, unexpected_error),
			}
		}
		else
		{
			unexpected_result!(get_mempolicy, result)
		}
	}
}
