...
pub struct QueryTypeCache<'cache, Record: Record>(LeastRecentlyUsedCache<'cache, QueryTypeCacheEntry<'cache, Record>>);

impl<'cache, Record: Record> QueryTypeCache<'cache, Record>
{
	#[inline(always)]
	...
	#[inline(always)]
	pub(crate) fn put_name_error<'message>(&mut self, query_name: EfficientCaseFoldedName, negative_cache_until: CacheUntil, record: StartOfAuthority<ParsedName<'message>>)
	{
	...
	#[inline(always)]
	pub(crate) fn put_no_data<'message>(&mut self, query_name: EfficientCaseFoldedName, negative_cache_until: CacheUntil, record: StartOfAuthority<ParsedName<'message>>)
	{
		self.put_absent(query_name, negative_cache_until, record)
	}
	
	#[inline(always)]
	fn put_absent<'message>(&mut self, name: EfficientCaseFoldedName, negative_cache_until: CacheUntil, record: StartOfAuthority<ParsedName<'message>>)
	{
		use self::QueryTypeCacheEntry::*;
