...
pub(crate) struct ResponseRecordSectionsParser<'message>
{
	...
	parsed_labels: RefCell<ParsedLabels>,
...
{
	#[inline(always)]
	pub(crate) fn new(data_type: DataType, end_of_message_pointer: usize, message_header: &'message MessageHeader, parsed_labels: ParsedLabels) -> Self
	{
		Self
		{
			data_type,
			...
			parsed_labels: RefCell::new(parsed_labels),
	...
	#[inline(always)]
	pub(crate) fn parse_answer_authority_and_additional_sections<RRV: ResourceRecordVisitor<'message>>(&mut self, next_resource_record_pointer: usize, query_name: &'message WithCompressionParsedName<'message>, answer_quality: AnswerQuality, answer_section_resource_record_visitor: &mut RRV) -> Result<(usize, AnswerOutcome), DnsProtocolError>
	{
		...
		let (next_resource_record_pointer, answer_outcome) = self.parse_authority_section(next_resource_record_pointer, canonical_name_chain, answer_quality, answer_section_has_at_least_one_record_of_requested_data_type)?;
	...
	#[inline(always)]
	fn parse_answer_section<RRV: ResourceRecordVisitor<'message>>(&mut self, next_resource_record_pointer: usize, query_name: &'message WithCompressionParsedName<'message>, answer_section_resource_record_visitor: &mut RRV) -> Result<(usize, CanonicalNameChain<'message>, bool), DnsProtocolError>
	{
	...
	#[inline(always)]
	fn parse_authority_section(&mut self, next_resource_record_pointer: usize, canonical_name_chain: CanonicalNameChain<'message>, answer_quality: AnswerQuality, answer_section_has_at_least_one_record_of_requested_data_type: bool) -> Result<(usize, AnswerOutcome), DnsProtocolError>
	{
		...
		let parse_method = |resource_record: ResourceRecord| resource_record.parse_authority_section_resource_record_in_response(self.end_of_message_pointer, self.parsed_labels.borrow_mut(), &mut authority_resource_record_visitor, &self.response_parsing_state, &self.duplicate_resource_record_response_parsing);
		let next_resource_record_pointer = self.loop_over_resource_records(next_resource_record_pointer, number_of_resource_records, parse_method)?;

		let answer_outcome = authority_resource_record_visitor.answer_outcome(answer_quality.is_authoritative_answer(), answer_quality.has_nxdomain_error_code(), answer_section_has_at_least_one_record_of_requested_data_type);
	...
	fn parse_additional_section(&mut self, next_resource_record_pointer: usize) -> Result<usize, DnsProtocolError>
	{
		...
		let parse_method = |resource_record: ResourceRecord| resource_record.parse_additional_section_resource_record_in_response(self.end_of_message_pointer, self.parsed_labels.borrow_mut(), &mut discarding_resource_record_visitor, &self.response_parsing_state, &self.duplicate_resource_record_response_parsing);
		let next_resource_record_pointer = self.loop_over_resource_records(next_resource_record_pointer, number_of_resource_records, parse_method)?;
