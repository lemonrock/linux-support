...
{
	#[inline(always)]
	pub(crate) fn new(now: NanosecondsSinceUnixEpoch, data_type: DataType, end_of_message_pointer: usize, message_header: &'message MessageHeader, parsed_labels: ParsedLabels) -> Self
	{
		Self
		{
			now,
	...
	#[inline(always)]
	fn parse_answer_section<RRV: ResourceRecordVisitor<'message>>(&mut self, next_resource_record_pointer: usize, query_name: WithCompressionParsedName<'message>, answer_section_resource_record_visitor: &mut RRV) -> Result<(usize, CanonicalNameChain<'message>, bool), DnsProtocolError>
	{
	...
	fn parse_authority_section(&mut self, next_resource_record_pointer: usize, canonical_name_chain: CanonicalNameChain<'message>, answer_quality: AnswerQuality, answer_section_has_at_least_one_record_of_requested_data_type: bool) -> Result<(usize, AnswerOutcome, CanonicalNameChain<'message>), DnsProtocolError>
	{
		...
		let parse_method = |resource_record: ResourceRecord| resource_record.parse_authority_section_resource_record_in_response(self.now, self.end_of_message_pointer, self.parsed_labels.borrow_mut(), &mut authority_resource_record_visitor, &self.response_parsing_state, &self.duplicate_resource_record_response_parsing);
		let next_resource_record_pointer = self.loop_over_resource_records(next_resource_record_pointer, number_of_resource_records, parse_method)?;
	...
	fn parse_additional_section(&mut self, next_resource_record_pointer: usize) -> Result<usize, DnsProtocolError>
	{
		...
		let parse_method = |resource_record: ResourceRecord| resource_record.parse_additional_section_resource_record_in_response(self.now, self.end_of_message_pointer, self.parsed_labels.borrow_mut(), &mut discarding_resource_record_visitor, &self.response_parsing_state, &self.duplicate_resource_record_response_parsing);
		let next_resource_record_pointer = self.loop_over_resource_records(next_resource_record_pointer, number_of_resource_records, parse_method)?;
